...
PROFILE = ROOT / "data" / "engine" / "experimental-voice-profile.v1.json"
...
def validate() -> tuple[int, int]:
    ...
    if profile.get("schema_version") != "1.0.0":
        raise ValueError("voice profile schema version must be 1.0.0")
    if profile.get("profile_id") != "iberolab-sign-reading-voice-v1":
    ...
    if profile.get("classification") != "synthetic_reading_of_conventional_sign_values":
    ...
    engine_data = profile.get("engine", {})
    required_engine_values = {
    ...
    for key, value in required_engine_values.items():
        if engine_data.get(key) != value:
            raise ValueError(f"voice engine has invalid {key}")
    ...
    require_markers(
        "MVP audio policy",
        audio_policy,
        ("adapted sign tokens", "experimental acoustic recreation", "reconstructed Iberian pronunciation"),
    )
    ...
    require_markers(
        ...
        script,
        (
            'PROFILE_ID = "iberolab-sign-reading-voice-v1"',
            "SAMPLE_RATE = 24000",
            "hashString",
            "makeRandom",
            "buildPlan",
    ...
    for forbidden in ("speechSynthesis", "SpeechSynthesisUtterance", "fetch(", "XMLHttpRequest", "WebSocket"):
    ...
    require_markers(
        ...
        (
            ...
            "sintetizador de formantes determinista",
            "`s` y `ś`",
            "`r` y `ŕ`",
    ...
    require_markers(
        ...
        (
            ...
            "iberolab-sign-reading-voice-v1",
        ),
    )
    require_markers(
        "public methodology",
        methodology,
        (
            "Dos funciones sonoras, dos alcances",
            "Recreación experimental",
    ...
    require_markers(
        ...
        (
            "Audio local",
            "se calcula íntegramente en el dispositivo",
            "no envía texto ni audio a IberoLab",
        ),
    )
...
if __name__ == "__main__":
    ...
    print(
        ...
        f"{token_count} tokens; {source_count} sources; deterministic local synthesis; "
