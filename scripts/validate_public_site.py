...
INDEX=ROOT/"docs"/"index.html";HISTORY=ROOT/"docs"/"historia.html";METHODOLOGY=ROOT/"docs"/"metodologia.html";CONVERTER=ROOT/"docs"/"convertir.html";STYLE=ROOT/"docs"/"public-site.css";MARK=ROOT/"docs"/"assets"/"brand"/"iberolab-mark.svg"
...
def main():
    files=(INDEX,HISTORY,METHODOLOGY,CONVERTER,STYLE,MARK)
    missing=[str(p.relative_to(ROOT)) for p in files if not p.is_file()]
    if missing: raise ValueError(f"missing public files: {missing}")
    index=INDEX.read_text(encoding="utf-8");history=HISTORY.read_text(encoding="utf-8");method=METHODOLOGY.read_text(encoding="utf-8");converter=CONVERTER.read_text(encoding="utf-8");style=STYLE.read_text(encoding="utf-8");mark=MARK.read_text(encoding="utf-8")
    ...
    require(converter,("Escuchar lectura aproximada","experimentalReadingText","recurso didáctico y no una reconstrucción histórica",'href="historia.html"','href="metodologia.html"','href="academia.html"'),"converter")
    ...
    if "Escuchar en ibero" in converter: raise ValueError("authentic Iberian pronunciation claim detected")
    print("PUBLIC SITE VALIDATION OK: landing, history, methodology, academic links, contemporary mark and approximate-reading safeguards present.")
    return 0
