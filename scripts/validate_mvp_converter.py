...
def validate_web_files():
    ...
    page_markers=("Adaptación fonética experimental","No es una traducción a la lengua ibérica","Escuchar entrada en español","Escuchar aproximación sonora","Fluida moderna · voz natural","Técnica · síntesis reproducible","Compartir resultado","Recientes en este dispositivo","IberoLab no recibe este historial",'id="historySection"','HISTORY_KEY = "iberolab:mvp:recent:v1"',"HISTORY_LIMIT = 5","localStorage",'id="actionStatus"','id="recreationButton"','id="recreationMode"','wordLabel.className = "word-label"','searchParams.set("q", trimmed)',"navigator.share","No reconstruye cómo hablaban los íberos","iberolab-sign-reading-voice-v3","Forma ibérica atestiguada","Entrada española moderna","Los signos son formas normalizadas de referencia",'src="mvp-converter.js"','src="iberian-voice.js"','src="converter-ui.js"','data-example="hogar"','data-example="tierra"','data-example="mundo"','data-example="olivo"','data-example="mar"','dataset.mvpConverterReady = "true"')
    ...
    for forbidden in ("Escuchar lectura aproximada","approximateButton","experimentalReadingText","Escuchar en ibero","pronunciación auténtica ibérica"):
