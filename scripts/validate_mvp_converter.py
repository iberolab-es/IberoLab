...
def validate_web_files():
    ...
    page_markers=("Adaptación fonética experimental","No es una traducción al idioma ibérico","Escuchar entrada en español","Escuchar lectura aproximada","Compartir resultado","Recientes en este dispositivo","IberoLab no recibe este historial",'id="historySection"','HISTORY_KEY="iberolab:mvp:recent:v1"',"HISTORY_LIMIT=5","localStorage",'id="actionStatus"','wordLabel.className="word-label"','searchParams.set("q",trimmed)',"navigator.share","experimentalReadingText","recurso didáctico y no una reconstrucción histórica","Los signos son formas normalizadas de referencia",'src="mvp-converter.js"','src="converter-ui.js"','data-example="amor"','data-example="familia"','data-example="te quiero"','data-example="amistad"','data-example="hogar"','dataset.mvpConverterReady="true"')
